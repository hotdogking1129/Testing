	...
	public class test2 {
		...
		public static void main(String[] args) {
			...
			for(int i=0 ; i<graph.head.path.size() ; i++) {
				System.out.println(graph.head.path.get(i) + "\nTotal distance = " + graph.head.total.get(i) + "\nTotal price = " + graph.head.price.get(i) + "\n");
			}
			System.out.println("\n\nThe minimun path is :" + graph.head.path.get(graph.minDistance(graph.head)));
			System.out.println("Distance :" + graph.head.total.get(graph.minDistance(graph.head)));
			System.out.println("Price: " + graph.head.price.get(graph.minDistance(graph.head)));
		}
	}
